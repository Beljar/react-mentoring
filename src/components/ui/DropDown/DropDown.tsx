...
const DropDown: React.FC<Props> = ({ value, options, className, onChange }) => {
  const Control = (props) => <Components.Control {...props} className={scss.control} />;
  const MenuList = (props) => <Components.MenuList {...props} className={scss.menu} />;
  const Option = (props) => {
    console.log(props);
    return <Components.Option {...props} className={cn(scss.option, { [scss.selected]: props.isSelected })} />;
  };
